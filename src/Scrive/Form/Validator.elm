...
import Scrive.Tag exposing (Tag)
...
tagValidator : { isNew : Bool, nameId : String } -> UG.TagList -> Validator Form.Error Tag
tagValidator { isNew, nameId } currentTags =
    let
        nameField  = if isNew then FE.NewTagName  else FE.NameOfTagWith  { name = nameId }
        valueField = if isNew then FE.NewTagValue else FE.ValueOfTagWith { name = nameId }
    in Validate.all
...
addressValidator =
    ...
    in Validate.all
        [ V.ifTrue
            ...
            ) <| FE.make FE.AddressEmail "E-mail should be specified when preferred contact method is set to e-mail"
        , V.ifTrue
            (\addr ->
                addr.preferredContactMethod == CD.PC_Post && notSpecified addr.address
            ) <| FE.make FE.AddressStreet "Street address should be specified when preferred contact method is set to post`"
        , V.ifTrue
            (\addr ->
                addr.preferredContactMethod == CD.PC_Phone && notSpecified (Maybe.map CD.phoneToString <| addr.phone)
            ) <| FE.make FE.AddressPhone "Phone should be specified when preferred contact method is set to phone"
        , V.ifFalse (.email >> Maybe.map CD.emailToString >> Maybe.map V.isValidEmail >> Maybe.withDefault True)
            <| FE.make FE.AddressEmail "Specified e-mail is in invalid format"
        , V.ifFalse (.phone >> Maybe.map CD.phoneToString >> Maybe.map V.isValidPhone >> Maybe.withDefault True)
            <| FE.make FE.AddressPhone "Specified phone is in invalid format"
        ]
