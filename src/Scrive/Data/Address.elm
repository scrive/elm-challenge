module Scrive.Data.Address exposing
    ...
    , fromDraft
    , phoneToString
    , preferredContactFromOption
    , preferredContactToOption
    , preferredContactToString
    , preferredWaysToContact
    , setDraftValue
    , toDraft
    , zipCodeToString
    )
...
type ZipCode
    = ZipCode String -- TODO: Improve later


type Phone
    = Phone String -- TODO: Improve later
...
allFields =
    [ F_Email, F_Phone, F_CompanyName, F_StreetAddress, F_ZipCode, F_City, F_Country ]


preferredWaysToContact : List PreferredContact
preferredWaysToContact =
    [ PC_Email, PC_Phone, PC_Post ]
...
setDraftValue drec field value =
    case field of
        F_Email ->
            { drec | email = Just value }

        F_Phone ->
            { drec | phone = Just value }

        F_ZipCode ->
            { drec | zip = Just value }

        F_CompanyName ->
            { drec | companyName = Just value }

        F_StreetAddress ->
            { drec | address = Just value }

        F_City ->
            { drec | city = Just value }

        F_Country ->
            { drec | country = Just value }
